...
__global__ void kernels::compute_checksums(float* matrix, int rows, int cols, int reduction_direction, float* checksum)
{
    extern __shared__ float shared_data[]; // shared memory for intermediate sums, as big as the blockdim vector

    int index_orthogonal = reduction_direction == REDUCE_ALONG_COL ? blockIdx.x : blockIdx.y; // 1 block per row/col checksum, the ortho direction is just the block index in that direction

    float sum = 0.0f;

    // info along the reduction direction
    int index_reduction = reduction_direction == REDUCE_ALONG_COL ? threadIdx.y : threadIdx.x;
    int blockDim_reduction = reduction_direction == REDUCE_ALONG_COL ? blockDim.y : blockDim.x;
    int limit_reduction = reduction_direction == REDUCE_ALONG_COL ? rows : cols;
    ...
    {
        sum += reduction_direction == REDUCE_ALONG_COL ? matrix[i * cols + index_orthogonal] : matrix[index_orthogonal * (cols + 1) + i];
    }
    ...
    if (index_reduction == 0)
    {
        ...
        if (reduction_direction == REDUCE_ALONG_COL)
        {
